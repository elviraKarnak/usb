...
if ( ! function_exists( 'usb_v2_content_nav' ) ) {
    ...
    ?>
            <div id="<?php echo esc_attr( $nav_id ); ?>" class="d-flex mb-4 justify-content-between">
                <div><?php next_posts_link( '<span aria-hidden="true">&larr;</span> ' . esc_html__( 'Older posts', 'usb-v2' ) ); ?></div>
                <div><?php previous_posts_link( esc_html__( 'Newer posts', 'usb-v2' ) . ' <span aria-hidden="true">&rarr;</span>' ); ?></div>
            </div><!-- /.d-flex -->
    <?php
        } else {
                    ...
                    <?php
                        comment_reply_link(
                            array_merge(
                                $args,
                                array(
                                    'reply_text' => esc_html__( 'Reply', 'usb-v2' ) . ' <span>&darr;</span>',
                                    'depth'      => $depth,
    ...
    if( $viewed_products ) {
        ...
        if ( $get_viewed_products->have_posts() ) :
             ...
             echo $title = '<h3>Recently Viewed Products</h3>';
                  while ( $get_viewed_products->have_posts() ) : $get_viewed_products->the_post();             
...
function display_custom_quantity_price_after_title() {
    ...
    // Fetch the custom price for quantity range 500
    $price_500 = get_post_meta( $product_id, '_custom_quantity_set499', true );

    // Display the price if it exists
    if ( !empty($price_500) ) {
        echo '<p class="custom-price-range">' . wc_price($price_500) . '</p>';
    }
}
...
function enqueue_script_on_users_list_page( $hook ) {
    ...
    $ver = '1.0'; // Define a version
    $vert = time(); // Define a version

    // Register styles and scripts
    wp_register_style( 'select_2_css_user', $uri . 'select2.min.css', [], $ver );
    wp_register_style( 'admin_style_modification', $uri . 'admin_style.css', [], $vert );

    wp_register_script( 'select_2_js_user', $uri . 'select2.min.js', ['jquery'], $ver, true );
    wp_register_script( 'admin_script_modification', $uri . 'admin_script.js', ['jquery'], $vert, true );
