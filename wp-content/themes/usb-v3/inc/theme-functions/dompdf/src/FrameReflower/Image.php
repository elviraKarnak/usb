...
class Image extends AbstractFrameReflower
{
    ...
    function get_min_max_width()
    {
        ...
        if (Helpers::is_percent($width)) {
            ...
            $width = ((float)rtrim($width, "%") * floatval($t)) / 100; //maybe 0
        } else {
        ...
        if (Helpers::is_percent($height)) {
            ...
            $height = ((float)rtrim($height, "%") * floatval($t)) / 100; //maybe 0
        } else {
            // Don't set image original size if "%" branch was 0 or size not given.
            // Otherwise aspect changed on %/auto combination for width/height
            // Resample according to px per inch
        ...
        if ($width == 0 || $height == 0) {
            ...
            if ($width == 0 && $height == 0) {
                ...
                $width_forced = false;
                $width = (floatval($height) / floatval($img_height)) * floatval($img_width); //keep aspect ratio
            }
        }
