...
if ( $is_IE ) :
	...
	if ( has_post_thumbnail() ) {
		...
		if(isset($_GET['color_att'])):
			$color_att=$_GET['color_att'];			
			$image_id = get_post_meta($post->ID, 'custom_image_upload_field_'.$color_att, true);
			if($image_id){
				$image   = wp_get_attachment_image( $image_id,'shop_single' );
				$image_title = esc_attr( get_the_title( $image_id ) );
				$image_link  = wp_get_attachment_url( $image_id );
				list( $magnifier_url, $magnifier_width, $magnifier_height ) = wp_get_attachment_image_src( $image_id, 'full' );
